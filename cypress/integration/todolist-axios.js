describe('#todolist-axios',()=>{
    beforeEach(() => {
        cy.visit('/todolist-axios');
    });
    ...
    it.only('should insert new todo',()=>{
        ...
        //verify list is empty
        const nonExistent = Cypress.$('.tasklist__content-row');
        cy.log(nonExistent.length)
        if(nonExistent.length > 0){
            cy.get('.todolist__content ul')
            .then(contentRows => {
                const listingCount = Cypress.$(contentRows).length;
